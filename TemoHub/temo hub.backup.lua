...
GlowA.Size = UDim2.new(0, 180, 0, 180)
GlowA.Position = UDim2.new(0, -40, 0, -40)
GlowA.BackgroundColor3 = palette.accent
GlowA.BackgroundTransparency = 0.8
GlowA.BorderSizePixel = 0
GlowA.ZIndex = 0
GlowA.Parent = Container
makeRounded(GlowA, 90)

local GlowB = Instance.new("Frame")
GlowB.Size = UDim2.new(0, 200, 0, 200)
GlowB.Position = UDim2.new(1, -120, 1, -120)
GlowB.AnchorPoint = Vector2.new(1, 1)
GlowB.BackgroundColor3 = palette.stroke
GlowB.BackgroundTransparency = 0.82
GlowB.BorderSizePixel = 0
GlowB.ZIndex = 0
GlowB.Parent = Container
makeRounded(GlowB, 100)
...
TabBar.BackgroundColor3 = palette.background
...
local function setActiveTab(name)
    ...
    local newData = tabs[name]
    if not newData then return end

    if TabBar.AbsoluteSize.X == 0 then
        task.defer(function()
            setActiveTab(name)
        end)
        return
    ...
    local newPage = newData.Page
    local oldPage = activePage
    activePage = newPage

    for tabName, data in pairs(tabs) do
        local isActive = tabName == name
        data.Button.BackgroundColor3 = isActive and palette.accentDark or palette.panel
        data.Button.TextColor3 = isActive and Color3.new(1, 1, 1) or palette.text
    end

    if TabIndicator then
        local width = newData.Button.AbsoluteSize.X
        if width <= 0 then
            task.defer(function()
                setActiveTab(name)
            end)
        ...
        local relX = newData.Button.AbsolutePosition.X - TabBar.AbsolutePosition.X
        TabIndicator.Size = UDim2.new(0, width, 0, 3)
        TabIndicator.Visible = true
        tween(TabIndicator, TweenInfo.new(0.18, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
            Position = UDim2.new(0, relX, TabIndicator.Position.Y.Scale, TabIndicator.Position.Y.Offset),
        })
    end

    if oldPage then
        tween(oldPage, TweenInfo.new(0.16, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
    ...
    tween(newPage, TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
...
local function createTab(tabName)
    ...
    button.MouseEnter:Connect(function()
        ...
        tween(button, TweenInfo.new(0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
            BackgroundColor3 = palette.tabHover or Color3.fromRGB(30, 35, 45),
        })
    end)
...
local function applyTheme(name)
    ...
    GlowA.BackgroundColor3 = theme.accent
    GlowB.BackgroundColor3 = theme.stroke
